...
def evaluate_regression(y_true, y_pred):
    ...
    # R^2 Score
    # TODO: Calculate R^2
    rss = np.sum([(y_true[i] - y_pred[i])**2 for i in range(len(y_true))])
    tss = np.sum([(y_true[i] - np.mean(y_pred))**2 for i in range(len(y_true))])

    r_squared = 1 - rss/tss

    # Root Mean Squared Error
    # TODO: Calculate RMSE
    rmse = np.sqrt(1/len(y_true)*rss)

    # Mean Absolute Error
    # TODO: Calculate MAE
    mae = 1/len(y_true)*np.sum([np.abs(y_true[i] - y_pred[i]) for i in range(len(y_true))])
